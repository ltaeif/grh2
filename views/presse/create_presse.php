...
$typeError = null;
	...
	/*if ( !empty($_POST)) {
		...
		if ($valid) {
			...
			//add langue
			$sql = "INSERT INTO presse_lang (titre,description,lang) values(?, ?, ?)";
			$q = $pdo->prepare($sql);
			$q->execute(array('NOW()'));
			Database::disconnect();
...
function bytesToSize(bytes) {
  ...
  <div class="form-actions">
	<a class="btn" href="index.php?page=presse&action=index&pagi=<?=$pagi;?>">Back</a>
  </div>
					...
					<div align="center">
		    			...
		    			<h3>Créer un article presse</h3>
		    		<div id="alert">
		  
					</div>
					<form class="form-horizontal" action="processuploadpresse.php" method="post" enctype="multipart/form-data" id="MyUploadForm">
					  
					    
					  <div class="control-group <?php echo !empty($videoError)?'error':'';?>">
					    <label class="control-label">Image de la presse</label>
					    <div class="controls">
