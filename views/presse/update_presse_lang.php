...
<?php 
	...
	if ( !empty($_POST)) {
		...
		if ($valid) {
			...
			//add langue
			$sql = $sql = "UPDATE "._DB_PREFIX_."presse_lang  set titre = ?, description = ?   WHERE id = ?";
			
			$q = $pdo->prepare($sql);
			$q->execute(array($titre,$description,$id));
			Database::disconnect();
			header("Location: index.php?page=presse&action=index&pagi=".$pagi."#".$id_presse);
		}
	}
	 else {
		$pdo = Database::connect();
    ...
    $(function() {
      ...
      $('form').on('submit', function (e) {
        ...
        //alert($('.summernote').code());
					...
					<h3>Mise à jour description du presse </h3>
		    		
					<form class="form-horizontal" action="index.php?page=presse&action=update_presse_lang&id=<?=$id;?>&id_presse=<?=$id_presse;?>&pagi=<?=$pagi;?>" method="post"  id="MyUploadForm">
					  ...
					  <div class="control-group <?php echo !empty($titreError)?'error':'';?>">
					    <label class="control-label">Titre du presse</label>
					    <div class="controls">
