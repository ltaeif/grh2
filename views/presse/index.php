					   ...
					   $sql = 'SELECT p.* FROM '._DB_PREFIX_.'presse p 
	 				   ...
	 				   foreach ($pdo->query($sql) as $row) {
								...
								<div class="box" style="background:url(uploads_presse/'.$thumb_prefix.$row['image'] . ') no-repeat 50% 50%;">
								  <a title="modifier" href="#" onclick="location.href=\'index.php?page=presse&action=update_presse&id='.$id.'&pagi='.$pagi.'\'; "/><div class="overlay">
									<span class="search"> 
							   ...
							   echo '<td>
								...
								?>
								 ...
								 $sql = 'SELECT * FROM '._DB_PREFIX_.'presse_lang where id_presse = '.$row['id'].' ORDER BY id DESC';
									...
									echo '<td >';
									echo '<a class="btn btn-success" href="index.php?page=presse&action=update_presse_lang&id='.$row1['id'].'&id_presse='.$row['id'].'&pagi='.$pagi.'">Mise à jour</a>';
									echo '&nbsp;';
									echo '<a class="btn" href="index.php?page=presse&action=read&id='.$row1['id'].'&id_presse='.$row['id'].'&pagi='.$pagi.'">Voir</a>';
									echo '</td>';
									echo '</tr>';
								...
								echo '&nbsp;';
							   	echo '<a class="btn btn-danger" href="index.php?page=presse&action=delete&id='.$row['id'].'&pagi='.$pagi.'">Delete</a>';
